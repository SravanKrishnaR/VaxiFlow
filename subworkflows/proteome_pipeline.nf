...
workflow PIPELINE_PROTEOME {
    take:
        protein_sequences_ch

    main:
        if (params.mode == 'unfiltered') {

        PROTEOME_IDS_ch = PROTEOME_IDS(protein_sequences_ch)
        
        if (params.end_at == 'PROTEOME_IDS') {
            return
        }

        VFDB_INPUT_ch = protein_sequences_ch.join(PROTEOME_IDS_ch.proteome_csv).map{ name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        HUMAN_HOMOLOGS_INPUT_ch = protein_sequences_ch.join(VFDB_ch.vfdb_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        ALGPRED_INPUT_ch = protein_sequences_ch.join(HUMAN_HOMOLOGS_ch.human_homologs_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        SIGNALP_INPUT_ch = protein_sequences_ch.join(ALGPRED_ch.algpred_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        DEEPLOCPRO_INPUT_ch  = protein_sequences_ch.join(SIGNALP_ch.signalp_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        TOXINPRED_INPUT_ch     = protein_sequences_ch.join(DEEPLOCPRO_ch.deeplocpro_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        PHOBIUS_INPUT_ch  = protein_sequences_ch.join(TOXINPRED_ch.toxinpred_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        PROTPARAM_INPUT_ch = protein_sequences_ch.join(PHOBIUS_ch.phobius_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        B_CELL_INPUT_ch = protein_sequences_ch.join(PHOBIUS_ch.phobius_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        MHC_II_INPUT_ch = protein_sequences_ch.join(PHOBIUS_ch.phobius_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
        ...
        MHC_I_INPUT_ch = protein_sequences_ch.join(PHOBIUS_ch.phobius_csv).map { name, proteome, proteome_csv -> tuple(name, proteome, proteome_csv)}
